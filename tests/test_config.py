...
def test_load_config_codex_model_reasoning_effort(tmp_path, monkeypatch):
    code_root = tmp_path / "code"
    data_dir = tmp_path / "data"
    code_root.mkdir()
    data_dir.mkdir()
    ...
    cfg = cfgmod.load(str(cfg_path))
    assert cfg.codex.model_reasoning_effort == "minimal"
...
def test_load_config_totp_file_transfer_group(tmp_path, monkeypatch):
    code_root = tmp_path / "code"
    data_dir = tmp_path / "data"
    code_root.mkdir()
    data_dir.mkdir()
    ...
    cfg = cfgmod.load(str(cfg_path))
    assert cfg.discord.totp_enforce_file_transfer is False
