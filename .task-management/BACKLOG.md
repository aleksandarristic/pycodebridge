...
- [TASK-0073] Sanitize TOTP before any DM audit write.
  - Context: DM command audit currently starts before `require_totp()` strips `--totp <code>`, and default redaction patterns do not cover TOTP arguments.
  - Goal: prevent TOTP codes from being persisted to audit logs or related command metadata.
  - Scope:
    - Start DM audit entries only after command text has been sanitized, or sanitize the metadata before audit start.
    - Add a default redaction pattern for `--totp <6 digits>` and equivalent normalized unlock syntax.
    - Cover DM `unlock`, DM admin create/delete, invalid TOTP, and replay paths.
  - Acceptance criteria:
    - No audit request, output, stderr, or bundled artifact includes raw TOTP codes.
    - Targeted audit/DM/TOTP tests pass.

- [TASK-0074] Apply redaction consistently to session JSONL and Codex error logs.
  - Context: `SessionJsonlLogger` and `codex_errors.log` record raw prompts, CLI args, JSONL, stderr, and output outside the audit redaction path.
  - Goal: make redaction policy consistent across all durable logs.
  - Scope:
    - Share `Redactor` with session JSONL and Codex error logging.
    - Redact prompt args, raw JSONL lines, stderr, Discord output chunks, and error-log payloads.
    - Consider changing `audit.redact` default to true, with docs migration notes if changed.
  - Acceptance criteria:
    - Tests prove common tokens/secrets/TOTP values are redacted in audit logs, session JSONL logs, and Codex error logs.
    - Existing audit and session-jsonl tests pass.
