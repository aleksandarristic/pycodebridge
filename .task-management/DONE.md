...
- [TASK-0137] Fix `codex.exit`/`LimitOverrunError` misreport when a single agent stdout/stderr line exceeds the asyncio stream buffer.
  - Completed: 2026-07-01
  - Notes: A live run at 2026-07-01T12:15:44Z crashed with `{"event": "codex.exit", "data": {"error": "Separator is not found, and chunk exceed the limit"}}` while Codex ran a broad `rg` search whose `command_execution` result embedded a huge single-line JSON payload. That string is CPython's stock message for `StreamReader.readuntil()`/`readline()` hitting asyncio's default 64 KiB buffer limit. [[TASK-0114]] investigated the identical error text on 2026-06-23 but misattributed it to Discord output-chunking/delivery; the actual bug — `AgentBackend.run`'s `async for raw in proc.stdout/stderr` in `codebridge/agents/base.py` had no `limit=` override and no handling for the `ValueError` that `readline()` raises once it internally resyncs past an oversized line — was untouched and reproduced. Fixed by raising the subprocess stream limit to 10 MiB (`_STREAM_LIMIT`) and switching both read loops to manual `readline()` calls that catch `ValueError` and skip the oversized line (documented as safe: `readline()` already drains its buffer past the line before raising) instead of returning it as a fatal process error. Added `tests/test_codex.py::test_runner_survives_oversized_stdout_line`, which forces a tiny `_STREAM_LIMIT` via monkeypatch and asserts the run finishes with `on_exit(err=None, ...)` and the subsequent line is still parsed.

- [TASK-0136] Add `!c sync-agent-env <repo>|--all` DM admin command to bulk-refresh `.agent-env.local.md` across managed repos.
  - Completed: 2026-07-01
  - Notes: Since `.agent-env.local.md` is only ever written once per repo, template improvements (like [[TASK-0135]]'s generic `{{TOOL:x}}`/`{{REPO_TOOL:x}}` placeholders) never reached repos seeded before the change. Added `Router.sync_agent_env_cache`/`sync_all_agent_env_caches`, which re-render everything above the `## Notes` heading from the current template and preserve everything from that heading onward byte-for-byte (skipping a repo instead of overwriting it if that heading is missing). Wired as a new DM-admin-only command (`admin_commands` set, dispatch branch, help text) in `codebridge/handlers/dm_admin.py`, not TOTP-gated (same risk tier as `repos`/`sessions`/`config`, since it only touches a gitignored cache file). Covered with router-level tests (create/update/skip/unchanged/bulk) and DM-handler tests (single repo, `--all`, missing-argument usage error); manually verified end-to-end against a scratch directory of repos.

- [TASK-0135] Unify the two local agent-env template files into one, with dynamically-extensible tool placeholders.
  - Completed: 2026-07-01
  - Notes: Deleted the stale, unwired `.agent-env.local.example.md` (superseded once `.agent-env.local.sample.md` and its rendering wiring were added); `.agent-env.local.sample.md` is now the single tracked template, already wired as the default via `config.example.yaml`'s `repo_bootstrap.agent_env_template` and covered by tests. Replaced the fixed-dict placeholder mechanism (`_agent_env_template_vars`, plus a second, drifted hardcoded fallback in `_agent_env_content`) with a generic regex-based renderer in `Router._render_agent_env_template`: `{{TOOL:<command>}}` checks PATH via `shutil.which`, `{{REPO_TOOL:<relative-path>}}` checks a path inside the repo checkout, and `{{CHECKED_DATE}}` remains a fixed special case. Adding a new tracked tool is now a pure `.md` edit — no router.py change needed. `_render_agent_env_content` falls back to the tracked sample file (resolved relative to the package, not cwd) when `agent_env_template` is unset, so there is one source of truth for structure instead of two that could drift. Removed now-dead helpers (`_repo_tool_detail`, `_external_tool_detail`, `_combined_tool_detail`, `_gh_tool_detail`) and updated `tests/test_dm_admin_repo_lifecycle.py` accordingly. The template also gained a standing free-form "Notes" section, safe from regeneration since `bootstrap_agent_env_cache` only ever writes the file once (`if not env_path.exists()`).
...
- [TASK-0133] Add timeouts to internal git/gh subprocess helpers used by dispatch/task-close/worktree flows.
  - Completed: 2026-07-01
  - Notes: Found while investigating TASK-0132 (occasional "agent not responding" hangs): `_git`/`_git_output`/`_gh_output` in `dispatch/orchestrator.py`, `dispatch/closer.py`, and `services/worktree.py` called `proc.communicate()` with no timeout, so a stalled network op or an unexpected credential prompt would hang the awaiting task forever with nothing surfaced to Discord. Added a shared-pattern `_communicate_with_timeout` per file (60s default, kills the process on expiry and raises the file's existing error type), and set `GIT_TERMINAL_PROMPT=0`/`GH_PROMPT_DISABLED=1` so git/gh fail fast instead of blocking on stdin for credentials. Covered with a fake slow `git` binary in `tests/test_worktree_manager.py`, `tests/test_task_closer.py`, and `tests/test_orchestrator_flow.py`.
...
- [TASK-0115] Add gitignored local agent environment cache pattern.
  - Completed: 2026-06-24
  - Notes: Added `.agent-env.local.md` ignore rule, a tracked `.agent-env.local.example.md` template, and AGENTS guidance to treat local environment notes as advisory caches while probing required commands per run.
