...
7) Optional: enable TOTP for protected commands (all transports):
   ...
   - Channel commands that do NOT require TOTP: `help`, `status`, `stats`, `peek`, `updates`, `models`, `show`/`showrepo`, `changes`/`showchanges`, `ps`, `unlock [gh|all] status`, `lock [gh|all]`, and read-only git helpers (`git status|log|branches|show|diff|remote`)
