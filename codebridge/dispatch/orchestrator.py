...
class Orchestrator:
    ...
    async def _run_planning_step(
        ...
        result = await self._run_backend("claude", repo_path, task_branch, plan_prompt, handler, branch_is_base=True)
    ...
    async def _run_backend(
        ...
        try:
            ...
            return AgentResult(agent=agent, success=False, branch=worker_branch, error=str(exc))
        finally:
            await self._wt_manager.remove(wt_path)
